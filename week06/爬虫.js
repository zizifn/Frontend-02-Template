 ...
 * @type {Element[]}
 */
const nodeList = [...document.querySelector('#container').children]

var urls = nodeList.filter(node => node.getAttribute("data-tag")
    .match(/css/))
    .map(node =>
        ({ name: node.children[1].innerText, url: node.children[1].children[0].href }))
